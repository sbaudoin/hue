...
def check_status(request):
  ...
  try:
    ...
    if response.get('query_status') is None:
      response['query_status'] = {}
    if response['status'] == 0:
      response['query_status']['status'] = response['query_status']['status']
    elif response['status'] == 'expired':
      response['query_status']['status'] = 'expired'
    else:
      response['query_status']['status'] = 'failed'
    ...
    if notebook['type'].startswith('query') or notebook.get('isManaged'):
      ...
      if nb_doc.can_write(request.user):
        ...
        diffs = [k for k in response['query_status'] if response['query_status'][k] != nb['snippets'][0].get(k)]
        if diffs:
          nb['snippets'][0].update(response['query_status'])
...
def get_logs(request):
  ...
  response['progress'] = db.progress(notebook, snippet, logs=full_log)
  response['jobs'] = jobs
